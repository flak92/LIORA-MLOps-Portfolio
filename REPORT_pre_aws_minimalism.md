...
project that is one host, one asset, four images and six services, is each
...
|---|---|---|
| the lift | one Linux instance (Amazon EC2) with one durable volume (Amazon EBS) | `docker compose`, the Makefile, a hand typing `make all`, the page behind the tunnel — the same stack, moved |
...
| the image carries the code | the image registry (Amazon ECR) for the four images | the same — the images carry the code and the mounts the four stores, here already |
...
| the image registry | none — the host builds the four images itself, as a bare recursive clone does here | no: the images carry the code, so elsewhere the service that runs the tasks pulls each by address; one image per module is one address each, and the registry is the one place the role-deciding service and command are not | § Infrastructure seats, the question *where does the image live?*; `module_skills/skill_asset_containers.md` § The topology |
| the state machine | a shell script calling the service's task API in the Makefile's order — the order already exists | no: a script would hold the order a second time in words the provider does not read; the state machine is the provider's form of the visible list in `all:`, `data-all:`, `features-all:` and `ml-all:` — the order moves as a rename, a second asset is one more iteration of the Map, and the Makefile stays the developer interface | § The Makefile is the developer interface; § Infrastructure seats; § Rejected forms, the batch service row |
...
| the copy's four prefixes | one prefix, the whole working tree copied as it is | no: the four prefixes are the four `store_*` roots — the three snapshots among them — the tree already keeps apart — raw, artifacts, runs, status — copied whole with their keys the descriptors' own paths; one prefix would blur the one family written once and never keyed by a run, the raw tree, with the two keyed by the run and the one, `status/`, rewritten by it | § The asset folder is a prefix, read forward |
