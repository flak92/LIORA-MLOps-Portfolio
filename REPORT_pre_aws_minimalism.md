...
|---|---|---|---|
| the instance and the volume | nothing — a host with a disk is the floor of the lift | no: the `store/<content>/` folders need a disk that outlives a container, and the database file needs a block device for its whole-file lock | § Rejected forms, the shared network filesystem row; `module_data/skills/skill_candle_canonicalisation.md` § 15 |
