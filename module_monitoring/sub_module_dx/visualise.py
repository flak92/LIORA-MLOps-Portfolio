...
def load_provenance_stamp(output_relative: str) -> tuple[str, str]:
    ...
    changed nothing except the drawn page.

    Walking keeps --check honest across a commit that carries only the page. A
    stamp naming HEAD would move the moment the page was committed, so the next
    run would produce different bytes for an identical tree.
    """
    if _git("rev-parse", "--is-shallow-repository").strip() == "true":
        raise VisualisationError(
            ...
            "  fix: fetch the whole history; a depth-limited clone cannot be stamped."
        )
...
def _validate_keys(where: str, given, allowed) -> None:
    ...
    if unknown:
        raise VisualisationError(
            ...
            f"  fix: correct the spelling, or drop the key — an unknown key is never ignored silently."
        )
...
def render_html(template_text: str, block: str) -> str:
    ...
    if template_text.count(config.STRUCTURE_BEGIN_MARKER) != 1 or \
        ...
        raise VisualisationError(
            ...
            f"so this file cannot tell which region it owns.\n  fix: leave exactly one pair."
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Draw the tracked git tree into files_and_folders_visualisation.html.")
    ...
    out = config.FILES_AND_FOLDERS_VISUALISATION_HTML_PATH
    if arguments.check:
        ...
        print(f"{out.relative_to(config.REPO_ROOT)} is stale: the tree has moved since it was written.\n"
              ...
              f"  fix: run `make {config.MAKE_TARGET}` and commit the result.")
        return 1
