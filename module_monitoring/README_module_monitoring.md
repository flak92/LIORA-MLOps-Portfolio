...
| piece | entry | does |
|---|---|---|
| the server | `make monitoring-dashboard`, `make monitoring-asset ASSET=<TICKER>` | one server, two roles by `ASSET`: the dashboard, or one asset container reporting itself |
| the DevOps panel | `make monitoring-devops` | `sub_module_devops`: the containers, networks and volumes, and the one container that holds the docker socket |

`make on` brings the three residents up
together, in this module's image, and prints the address.
