...
`module_skills/glossary.md` § DevOps panel enumerates them. What
...
| `window_start_utc` | it is a config constant, not a fact about the data — question 4 would cut it | no, and this is where question 4 shows it must come last: it is the one fixed reference against which a venue's `first_observation_utc` reads as a provider's missing history | `status.py`, `DATA_WINDOW_START_UTC` |
| `download_cadence_minutes` | it is `MINUTES_PER_DAY`, a constant | no: the Pipeline tab's observation-lag and measurement-age warnings read it, and a cadence is never a literal in a page — the seat of the constant is the payload precisely so the page has none | `module_monitoring/data.js`, `ageCell` |
| `window_end_utc` | the page could take the newest `last_observation_utc` and add one grid step | **yes, it goes** — and it should have gone before it was ever read: it is that maximum over the whole basket, while `venue_block` judges *every asset against its own canonical end*, so with two assets it names a window that all but the newest do not have. Its one reader is the envelope line | `status.py`, the window comment above `venue_block`, and the report's own window comment in `main` |
...
| `canonical_source[].row_count`, `.last_observation_utc` | nothing | no: the size and the end of the series research actually consumes; the Pipeline tab's observation lag reads the second | `module_monitoring/data.js`, `ageCell` |
