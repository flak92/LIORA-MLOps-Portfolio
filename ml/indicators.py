...
def asof_index(decision_ts: np.ndarray, level_open_ts: np.ndarray, tf_ms: int) -> np.ndarray:
    """Index of the last CLOSED bar of a level at each decision_ts.

    A bar [open, open + tf) is available from its close time onward, so
    side="right" on close times yields exactly the last closed bar. Causality
    is asserted, not assumed.
    """
    close_ts = level_open_ts + tf_ms
    idx = np.searchsorted(close_ts, decision_ts, side="right") - 1
    assert idx.min() >= 0, "decision before the first closed bar of the level"
