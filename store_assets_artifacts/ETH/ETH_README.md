...
| `ETH_parameters.json` | the one parameters file: what the search chose | 370 B |
...
The OHLCV lives in `ETH_research_ohlcv.duckdb` beside this file — the market object the whole chain reads, resident in the folder and outside the manifest above, because its size moves with every top-up and this file is promised byte-reproducible.
