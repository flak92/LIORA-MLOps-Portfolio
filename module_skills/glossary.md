...
| the status store: where every snapshot lands, tracked so a fresh clone opens on real numbers | `store/status/`, `STORE_STATUS_DIR`; `DATA_STATUS_JSON_PATH`, `FEATURES_STATUS_JSON_PATH`, `ML_STATUS_JSON_PATH` in the configs of the modules that write and read them, `SKILLS_STATUS_JSON_PATH` in the crawler's | `data_status.json`, `features_status.json`, `ml_status.json`, `skills_status.json` | the page's footer names the four snapshots | a snapshot written into `module_monitoring/` or any other module's directory |
...
| concept | code | artifact key | UI label | never |
|---|---|---|---|---|
| the crawler: the canon's sub-module that reads the files a hand lists against the written rules, writes a report a hand reads, and its snapshot — it gates nothing, edits nothing and commits nothing | `sub_module_scalability_crawler`; `make skills-crawl`, `make tmux-skills-crawl` in the tmux session `skills-crawl` | — | — | a linter, a check, a gate, a CI step; a web crawler, walking sites or services to gather what they hold |
| the list: a path from the root per line, a line ending in `/` every file under it but `__pycache__` and the reports | `to_crawl.txt`, `TO_CRAWL_TXT_PATH`, `load_crawl_paths()` | — | — | a list derived from the tree; the whole tree by default |
...
| the agent's command line — Claude Sonnet 5 in its user's own session, no tools, one fresh session per file | `AGENT_COMMAND`, `AGENT_TIMEOUT_MINUTES` | — | — | an API key; a model named outside the tuple; an interactive session |
| a file's report: one entry per crawl, the heading `## <YYYY-MM-DD HH:MM> UTC · <model> · <short commit>` and the answer as it came | `reports_after_crawled_files/<path>.md`, `REPORTS_AFTER_CRAWLED_FILES_DIR`, `report_path()`, `write_report_entry()`, `REPORT_HEADING_PATTERN` | `report` | report | a report overwritten or summarised; a report in a store; a dated review of the root, `REPORT_<subject>.md` (§ Documentation ownership) |
