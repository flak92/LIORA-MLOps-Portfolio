...
| a file's report: one entry per crawl, the heading `## crawled <YYYY-MM-DD HH:MM> UTC · <vendor> · <labels> · <short commit>` and the answer as it came | `reports_after_crawled_files/<path>.md`, `REPORTS_AFTER_CRAWLED_FILES_DIR`, `report_path()`, `write_report_entry()`, `REPORT_HEADING_PREFIX` | `report` | report | a report overwritten or summarised; a report in a store; a dated review of the root, `REPORT_<subject>.md` (§ Documentation ownership) |
| the menu: the gum header, the choice of one action and the prompts after it | `main()` and `_gum()` in `crawl.py`; `make skills-crawl` | — | Scalability crawler; `<n> path(s) listed`; action; crawl, add a path, remove a path; vendor; files to crawl; path to remove | a second menu after the action — the program ends; a remembered choice, a theme or colour beyond the header, a spinner, a crawl scheduled or detached |
