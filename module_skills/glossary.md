...
| mean absolute SHAP value (SHapley Additive exPlanations, `methodology_ml.md` § 13 [13]) per feature column of a validation fold's booster, over the fold's scoring rows and the three classes, in margin space, unweighted | `mean_abs_shap_importance()` | `mean_abs_shap_importance` | mean \|SHAP\| | `shap` as a key; SHAP importance (a method, not a quantity) |
| the rise of a validation fold's weighted log-loss when one column is permuted (Breiman's permutation importance, § 13 [12]) — one permutation per fold, applied to each column in turn | `permutation_logloss_delta_importance()` | `permutation_logloss_delta_importance` | permutation Δ log-loss | MDA, mean decrease accuracy, a per-column draw |
| the three importances of every validation booster, the block the page takes its cross-fold means from | `validation_importance_block()` | `validation_importance` with `fold_2` … `fold_4`, each holding the three keys above | FEATURE SET — the three columns of its tables | an importance of the final-holdout booster; a mean over folds in the payload |
| the columns the model saw — the asset's feature set as feature ids, timeframe-major | `feature_columns` of `load_xy()` | `feature_columns` | in set ✓ | a column literal in the page |
...
| an indicator — one computation with exactly one integer parameter, glued to its token; its kernel carries the token's name in `module_features/indicators.py`, and its invariants — the kernel, the parameter word, the warm-up multiple, the fixed inputs — are one record of the register beside it | `ema<n>`, `sma<n>`, `rsi<n>`, `atr<n>`, `zscore<n>`, `range_position<n>`; `INDICATORS`, `module_features/indicators.py` | `indicator`, `parameter_word`, `parameter_bars` of a term | the token | `rsi_14`, `sma_200`, `bb20`; a table of indicator facts in `config.py` beside the register; `kernel` for the operating system's accounting — that is the Lifecycle tab's word |
...
| a compose service that is one asset's container: resident, answering `/status`, and locally the place every per-asset stage runs | `asset-<ticker lowercase>` — one service per ticker under the file's `x-server` anchor | — | — | `asset-BTC`, `container-btc`, a one-off `run --rm` container beside a resident, a `restart:` policy, a published port |
