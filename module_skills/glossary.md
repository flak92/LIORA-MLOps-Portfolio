...
| how many timeframes must agree with the side | `MINIMUM_AGREEING_TREND_TIMEFRAMES`, `agreeing_trend_timeframe_count` | `minimum_agreeing_trend_timeframes` | at least `n` of `m` timeframes agree — both counts from the payload, the hierarchy's length among them | AGREE_MIN, n_agree, level (the crawler's `self_explaining_level` is a rating, § Scalability crawler) |
...
| passes of the feature-set search — one forward move and one backward move over the champion | `pass_count` | `pass_count` | passes | rounds (the boosting rounds' word), iterations (the Map's word); the crawler's pass (§ Scalability crawler) |
...
| whether a recorded search's inputs are still the asset's — false after a promotion, a retuning or a catalogue change, and the page then compares nothing | `feature_set_search_block()` | `inputs_current` | the note *the search predates the active set or its parameters*, in place of PROPOSALS and in the cross-section cell | stale (the crawler's word for a verdict older than the canon, § Scalability crawler), dirty, outdated; a guard that refuses the promotion |
...
|---|---|---|
| when the snapshot is written | `generated_at_utc` | the one timestamp of a payload; the crawler's snapshot carries none, its `measured_at_commit_utc` being a commit's time |
...
| the status store: where every snapshot and the crawler's review record land, tracked so a fresh clone opens on real numbers | `store/status/`, `STORE_STATUS_DIR`; `DATA_STATUS_JSON_PATH`, `FEATURES_STATUS_JSON_PATH`, `ML_STATUS_JSON_PATH` in the configs of the modules that write and read them, `SKILLS_STATUS_JSON_PATH` and `SKILLS_REVIEW_JSON_PATH` in the crawler's | `data_status.json`, `features_status.json`, `ml_status.json`, `skills_status.json`; `skills_review.json`, the crawler's review record, written by `crawl.py` alone | the page's footer names the three computational snapshots | a snapshot written into `module_monitoring/` or any other module's directory |
...
|---|---|---|
| the units, each holder the ones it uses: `MILLISECONDS_PER_SECOND`, `MILLISECONDS_PER_MINUTE`, `MILLISECONDS_PER_DAY` in Python; `MILLISECONDS_PER_SECOND` and `SECONDS_PER_MINUTE` in the page, `MILLISECONDS_PER_MINUTE`, `MINUTES_PER_HOUR` and `HOURS_PER_DAY` in the panel | `module_data/config.py`, `module_features/config.py`, `module_ml/config.py`; the browser's own in `module_monitoring/page.js` and `module_monitoring/sub_module_devops/containers.js`, which import no config | a unit is a unit; importing one across a boundary would drag the module behind it |
| `BYTES_PER_KIBIBYTE` | `module_data/config.py`, `module_ml/config.py`, `module_monitoring/config.py`; the browser's own in `module_monitoring/page.js` | the same |
...
| `rounded()` | `module_data/config.py`, `module_ml/config.py`, `module_skills/sub_module_scalability_crawler/config.py` | the status reports round the same way |
...
| the one selector component of both pages — the top tabs, the summary views, the ticker rows — each group wired by its `data-key` | `.pills`, `.pill`, `.pill--active`, `initPills()`, `PILL_HOOKS` in `page.js` | — | — | a second selector component; a pill, chip or tile standing for a badge |

## Scalability crawler

The names of `module_skills/sub_module_scalability_crawler` — the tracked tree
counted against its contract. The rules are
...
| the shape read condition by condition: a row holds when the invariant its evidence names reads zero, and is proven by hand when its evidence is a target | `SHAPE_EVIDENCE_BY_CONDITION`, `shape_block()` | `shape` with `condition`, `holds`, `evidence` | SHAPE — condition, holds, evidence | a condition a gate enforces; a row the contract does not hold |
