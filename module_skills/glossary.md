...
| an indicator — one computation with exactly one integer parameter, glued to its token; its kernel carries the token's name in `module_features/indicators.py`, and its invariants — the kernel, the parameter word, the warm-up multiple, the fixed inputs — are one record of the register beside it | `ema<n>`, `sma<n>`, `rsi<n>`, `atr<n>`, `zscore<n>`, `range_position<n>`; `INDICATORS` — `kernel`, `parameter_word`, `warmup_multiple`, `inputs` of a record, `module_features/indicators.py` | `indicator`, `parameter_word`, `parameter_bars` of a term | the token | `rsi_14`, `sma_200`, `bb20`; a table of indicator facts in `config.py` beside the register; `kernel` for the operating system's accounting — that is the Lifecycle tab's word |
...
| a feature definition — terms of one timeframe composed by the operators, with an optional normaliser; the timeframe-less half of a feature | one record of `FEATURE_CATALOGUE`; `feature_definition_name()`; `FEATURE_DEFINITION_OPERATORS` = (`minus`, `over`), `FEATURE_DEFINITION_NORMALISERS` = {`centered`: a record with `midpoint` and `half_range`, the two numbers the normaliser reads and nothing in code repeats} | `feature_definition` | definition | molecule, family, feature family, indicator (for a composite); `trend`, `momentum`, `volatility`, `structure`, `activity` — a category, not a computation |
