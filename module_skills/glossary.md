...
| the agent's command line — its user's own session, no tools, one fresh session per chosen file | `AGENT_COMMAND`, `AGENT_TIMEOUT_MINUTES` | — | — | an API key; a model named outside the tuple; an interactive session |
...
| the menu: the gum header, the choice of one action and the one prompt after it | `main()` and `_gum()` in `crawl.py`; `make skills-crawl` | — | Scalability crawler; action; crawl, add a path, remove a path; files to crawl; path to remove | a second menu after the action — the program ends; a remembered choice, a theme or colour beyond the header, a spinner, a crawl scheduled or detached |
