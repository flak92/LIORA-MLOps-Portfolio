...
| a runner — a compose service that is a role and a one-off: no command of its own, `docker compose run --rm -T <runner> python -m <module>.<stage> --tickers <TICKER>` supplies one and the container exits with the stage | `data`, `features`, `ml` — the `x-service` anchor and nothing else; the `run`, `fanout` and `basket` macros of the Makefile | — | — | `pipeline` (one runner for every module), a stage run by `exec` inside a resident, a runner with a `command:` |
