...
| the agent's command line — its user's own session, no tools, one fresh session per file | `AGENT_COMMAND`, `AGENT_TIMEOUT_MINUTES` | — | — | an API key; a model named outside the tuple; an interactive session |
