...
- `ingest.py` is basket-wide by design, and `AGENTS.md:31` lists "the
  basket-wide ingest" among the guards the mathematics requires — replacing it
  is a Values-level amendment (§1): the successor guard enters the list in
  the same commit. `bars.py` is the only writer of the ML
...
| snapshot keys: per-symbol `last_observation_utc` and `db_bytes` and the envelope's `download_cadence_minutes` in `data_status.json`; per-asset `manifest_complete` and `model_evaluation_modified_utc` in `ml_status.json` | `lag`, `fresh`, `stale`, `hash`, a per-asset timestamp in the README, a bare threshold literal in `containers.js` | the key grammar; the README stays byte-reproducible; the two snapshots are where the register one-liner already looks, so the keys are checked |
...
|---|---|---|---|
| data | `last_observation_utc`, and the observation lag as days and hours | the symbol's `canonical_source` row (stage 2 adds `last_observation_utc`); lag = client clock minus the last observation | `.badge--warn` above the download cadence — `download_cadence_minutes` from the snapshot envelope |
...
| **measured** | the measurement age in hours | the two snapshots' `generated_at_utc` against the client clock — Phase 1's truth signal: a stale snapshot looks stale, never fresh | `.badge--warn` above the download cadence, read from the `download_cadence_minutes` key of the `data_status.json` envelope — never a bare literal in the page: a change of cadence moves the threshold without a decision of its own |
...
The cadence is the data layer's own unit of download work — one UTC day,
`MINUTES_PER_DAY` in `module_data/lean.py` — published by `module_data/status.py`
as `download_cadence_minutes` (stage 2). When Phase 2 adds a scheduled
delivery loop, the same key carries the loop's cadence, and both badges follow
it.
...
| the review of the plan (2026-08-29) | approved with three conditions, folded in: the repealed guard is replaced by its successor in the same commit; the staleness threshold is the download cadence published by the data layer, never a bare number; the live layer is additive and the frozen window's byte-reproducibility is inviolable |
...
**Phase 2's step zero, named now and designed then: the live layer beside the
frozen research window.** One invariant is bound already, because it is a
constraint rather than a design: **the live layer is additive — it never
mutates a research artifact inside the frozen window, and the
byte-reproducibility of that window stays inviolable whatever Phase 2
decides.** Today's artifacts are bound to the research window and
byte-reproducible for an unchanged experiment (`RESEARCH_END_UTC` is frozen;
the README carries no timestamp; the determinism skill makes identical bytes
the standard); a continuously overwritten parquet is neither, so it can only
be a second, named family beside the research one. What step zero decides
is that family's shape — its own manifest rows, its own grammar for a moving
window, its own truth signal — and then amends the act, the methodology and
the determinism skill accordingly; whether the research window moves is not
on the table. Every carrier of *byte-reproducible* and *frozen research
