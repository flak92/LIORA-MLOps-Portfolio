...
the run record; the two snapshots. A stage never reads the copy: it knows a
...
| `data_status.json`, `ml_status.json`, `store_run_records/<run_id>/` | STORAGE — status and run objects | the run record under `runs/<run_id>/` and the two snapshots under `status/`, copied from the volume after the run, the page reading them on the volume as here; the snapshots' move off `module_monitoring/` is the five points of § What stays as it is, and why, the question of `skill_status_prefix.md` (`AGENTS.md` § Skills absent here, described) | a rename |
